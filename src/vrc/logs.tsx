...
export const VRCLogsLoader:FC =() => {
  ...
  useEffect(()=>{
    const reload = async()=>{
      ...
      const datas = await Promise.all(files.map(async(file)=>{
        ...
        const last_update_date = data.substring(len-1000,len-3).match(/([0-9]+\.[0-9]+\.[0-9]+ [0-9]+:[0-9]+:[0-9]+) [\s\S]+/)?.[1];
  ...
  useEffect(()=>{
    const update = async()=>{
      ...
      const avatar_id = data.match(/(?:Loading|Saving) Avatar Data:(avtr_.*)[\s\S]*?$/)?.[1];
      if (!user_id || !avatar_id) return;
      setCurrentAvatar(pv=>{
        if (pv && (pv.user_id&&pv.avatar_id)) return pv;
        setSystemLog((pv)=>[...pv,`[AvatarChanged/Log] Avatar:${avatar_id}`]);
    ...
    return ()=>clearInterval(interval);
  },[watchLog,config.profileAutoSwitch])
    ...
    async function f() {
      unlisten = await listen('avatar_change', event => {
        ...
        setCurrentAvatar(pv=>{
          ...
          setSystemLog((_pv)=>[..._pv,`[AvatarChanged/OSC] Avatar:${avatar_id}`]);
  ...
  }, [config.remote.listen,config.profileAutoSwitch])
  return <></>;
}